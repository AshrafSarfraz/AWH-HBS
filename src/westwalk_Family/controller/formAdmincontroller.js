// src/hr-system/controller/formAdminController.js
...
async function listForms(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Server error loading forms" });
  }
}
...
async function getFormById(req, res) {
  try {
    ...
    return res.status(500).json({ error: "Server error loading form" });
  }
}
...
async function updateForm(req, res) {
  try {
    ...
    const updated = await Form.findByIdAndUpdate(
      req.params.id,
      { formKey, displayName, description, includeEmployeeInfo, isLocked, active, fields },
...
async function getApprovalPriority(req, res) {
  try {
    ...
    const doc = await ApprovalPriority.findOne({ formName: formKey }).lean();
    if (!doc) return res.status(404).json({ error: "No approval chain for this form" });
    return res.json(doc);
  } catch (err) {
    console.error("getApprovalPriority error", err);
    return res.status(500).json({ error: "Server error loading approval chain" });
  }
}
...
async function upsertApprovalPriority(req, res) {
  try {
    ...
    if (!Array.isArray(sequence) || sequence.length === 0) {
      return res.status(400).json({ error: "sequence is required (array)" });
    }
    ...
    return res.status(500).json({ error: "Could not save approval chain" });
  }
}
...
async function deleteForm(req, res) {
  try {
    ...
    await ApprovalPriority.deleteOne({ formName: form.formKey });
